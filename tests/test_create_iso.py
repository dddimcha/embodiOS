...
class TestISOCreatorInitrd:
    ...
    def test_create_initrd_uses_list_arguments_not_shell(self):
        """
        Test that subprocess calls use list arguments,
        not shell=True (security fix)
        """
        creator = ISOCreator()
        ...
        with tempfile.TemporaryDirectory() as work_dir:
            ...
            with patch("subprocess.Popen") as mock_popen:
                ...
                ), "Should call Popen 3 times for pipeline"
                ...
                find_call = calls[0]
                assert find_call[0][0] == [
                    "find",
                    ".",
                ], "find should use list arguments"
                ...
                cpio_call = calls[1]
                assert cpio_call[0][0] == [
                    "cpio",
                    "-o",
                    "-H",
                    "newc",
                ], "cpio should use list arguments"
    ...
    def test_create_initrd_uses_cwd_for_directory_change(self):
        ...
        with tempfile.TemporaryDirectory() as work_dir:
            ...
            with patch("subprocess.Popen") as mock_popen:
                ...
                assert find_call[1]["cwd"] == str(
                    initrd_dir
                ), "cwd should point to initrd directory"
    ...
    def test_create_initrd_raises_error_on_gzip_failure(self):
        """
        Test that _create_initrd raises CalledProcessError
        when gzip fails
        """
        creator = ISOCreator()
    ...
    def test_create_initrd_pipeline_closes_pipes_correctly(self):
        """
        Test that subprocess pipes are closed correctly
        to avoid deadlocks
        """
        creator = ISOCreator()
    ...
    def test_create_initrd_no_command_injection_vectors(self):
        """
        Test that paths with special characters
        don't cause command injection
        """
        creator = ISOCreator()
        ...
        with tempfile.TemporaryDirectory() as base_dir:
            # Create a subdirectory with potentially dangerous chars
            # (in practice, temp dirs won't have these,
            # but this tests the safety)
