...
class ISOCreator:
    ...
    def _create_structure(self):
        ...
        dirs = ["boot/grub", "EFI/BOOT", "embodi", "isolinux"]
...
"""
    ...
    def _copy_embodi_files(self, model_path, arch):
        ...
        if kernel_src.exists():
            ...
            kernel_data = b"EMBODIOS_KERNEL" * 1000
            (self.work_dir / "embodi/kernel").write_bytes(kernel_data)
...
"""
        ...
        find_proc = subprocess.Popen(
            ["find", "."], cwd=str(initrd_dir), stdout=subprocess.PIPE
        )
        ...
        with open(output_path, "wb") as f:
            gzip_proc = subprocess.Popen(["gzip"], stdin=cpio_proc.stdout, stdout=f)
    ...
    def _create_boot_config(self, arch, features):
        """Create boot configuration"""
        config = {"arch": arch, "features": features, "version": "0.1.0"}

        import json

        config_json = json.dumps(config, indent=2)
        (self.work_dir / "embodi/config.json").write_text(config_json)
