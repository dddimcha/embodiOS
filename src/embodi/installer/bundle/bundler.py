#!/usr/bin/env python3
"""
EMBODIOS Bundler - Create deployable EMBODIOS bundles
"""
...
class EMBODIOSBundler:
    ...
    def create_bundle(self, model, output, target='bare-metal', arch='x86_64', 
        ...
        with tempfile.TemporaryDirectory() as work_dir:
            ...
            # Copy model
            shutil.copy2(model_path, work_path / 'embodi/model.aios')
    ...
    def _prepare_kernel(self, work_path, arch, features):
        """Prepare EMBODIOS kernel"""
        # In production, this would compile kernel with specific features
        # For now, copy pre-built or create dummy
        
        kernel_path = Path('build') / arch / 'embodi-kernel'
        
        if kernel_path.exists():
            shutil.copy2(kernel_path, work_path / 'embodi/kernel')
        else:
            # Create dummy kernel
            dummy_kernel = b'EMBODIOS_KERNEL_' + arch.encode() + b'\n'
            dummy_kernel += b'FEATURES: ' + ','.join(features).encode() + b'\n'
            dummy_kernel += b'\x00' * 1024 * 100  # 100KB dummy
            
            (work_path / 'embodi/kernel').write_bytes(dummy_kernel)
            
        # Make executable
        (work_path / 'embodi/kernel').chmod(0o755)
        
    def _create_config(self, work_path, model, arch, memory, features):
        """Create EMBODIOS configuration"""
        config = {
            'version': '0.1.0',
            'model': model,
            'arch': arch,
            ...
            'checksum': self._calculate_checksum(work_path / 'embodi/model.aios')
        }
        
        (work_path / 'config/embodi.json').write_text(json.dumps(config, indent=2))
...
menuentry "EMBODIOS - {arch}" {{
    set gfxpayload=keep
    linux /embodi/kernel model=/embodi/model.aios config=/config/embodi.json
    boot
}}

menuentry "EMBODIOS - Recovery Mode" {{
    set gfxpayload=text  
    linux /embodi/kernel model=/embodi/model.aios config=/config/embodi.json recovery
    boot
}}
"""
...
echo "Starting EMBODIOS..."
\embodi\kernel model=\embodi\model.aios config=\config\embodi.json
"""
            (work_path / 'EFI/BOOT/startup.nsh').write_text(startup_nsh)
            
    def _create_iso(self, work_path, output):
        """Create bootable ISO"""
        from embodi_installer.iso.create_iso import ISOCreator
    ...
    def _create_docker_bundle(self, work_path, output):
        ...
        # Create Dockerfile
        dockerfile = """FROM embodi-base:latest
COPY embodi /embodi
COPY config /config
CMD ["/embodi/kernel"]
"""
...
def main():
    ...
    parser = argparse.ArgumentParser(description='EMBODIOS Bundle Manager')
