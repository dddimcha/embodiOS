...
/* Type 1 Header (PCI-to-PCI Bridges) */
#define PCI_PRIMARY_BUS         0x18    /* Primary bus number */
#define PCI_SECONDARY_BUS       0x19    /* Secondary bus number */
#define PCI_SUBORDINATE_BUS     0x1A    /* Subordinate bus number */
...
/* Storage Subclasses */
#define PCI_SUBCLASS_SCSI       0x00    /* SCSI controller */
#define PCI_SUBCLASS_IDE        0x01    /* IDE controller */
#define PCI_SUBCLASS_FLOPPY     0x02    /* Floppy controller */
#define PCI_SUBCLASS_IPI        0x03    /* IPI controller */
...
#define PCI_SUBCLASS_SAS        0x07    /* SAS controller */
#define PCI_SUBCLASS_NVME       0x08    /* NVMe controller */

/* Bridge Subclasses */
#define PCI_SUBCLASS_HOST       0x00    /* Host bridge */
#define PCI_SUBCLASS_ISA        0x01    /* ISA bridge */
#define PCI_SUBCLASS_EISA       0x02    /* EISA bridge */
#define PCI_SUBCLASS_MCA        0x03    /* MCA bridge */
#define PCI_SUBCLASS_PCI        0x04    /* PCI-to-PCI bridge */
...
#define PCI_SUBCLASS_FIREWIRE   0x00    /* FireWire (IEEE 1394) */
 ...
 * Must be called before any other PCI functions
 *
 ...
 * Scans all buses and stores discovered devices
 *
 ...
 * Get device by index
 *
 * @param index Device index (0 to pci_device_count()-1)
 ...
 * @param subclass Subclass to match (PCI_ANY_CLASS for any)
 * @return Pointer to first matching device, or NULL
 */
 ...
 * The driver's probe function will be called for all matching devices
 *
 ...
 * The driver's remove function will be called for all bound devices
 *
...
/**
 * Get BAR address (memory or I/O base)
 *
 * @param dev Device to query
 * @param bar_index BAR index (0-5)
 * @return Base address, or 0 if BAR is not configured
 */
 ...
 * @param dev Device to query
 * @param bar_index BAR index (0-5)
 * @return Size in bytes, or 0 if BAR is not configured
 */
...
/**
 * Print all discovered PCI devices to console
 */
void pci_print_devices(void);

/**
 * Print PCI subsystem statistics
 */
...
/**
 * Get human-readable vendor name (for common vendors)
 *
 * @param vendor_id Vendor ID
 * @return Vendor name string, or "Unknown" if not in database
 */
