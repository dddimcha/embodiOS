...
class ISOCreator:
    ...
    def create_iso(self, model_path, output_path, arch='x86_64', features=None):
        """Create bootable NOVA ISO"""
        features = features or ['gpio', 'uart']
        
        with tempfile.TemporaryDirectory() as work_dir:
            self.work_dir = Path(work_dir)
            
            print(f"Creating NOVA ISO for {arch}")
            ...
            self._copy_nova_files(model_path, arch)
...
menuentry "NOVA - Natural Language OS" {
    set gfxpayload=keep
    linux /nova/kernel model=/nova/model.aios quiet
    initrd /nova/initrd.img
}

menuentry "NOVA - Safe Mode" {
    set gfxpayload=text
    linux /nova/kernel model=/nova/model.aios debug nosplash
    initrd /nova/initrd.img
}
"""
        ...
        isolinux_cfg = """DEFAULT nova
PROMPT 1
TIMEOUT 50

LABEL nova
    KERNEL /nova/kernel
    APPEND model=/nova/model.aios quiet
    
LABEL nova-debug
    KERNEL /nova/kernel  
    APPEND model=/nova/model.aios debug nosplash
"""
        (self.work_dir / 'isolinux/isolinux.cfg').write_text(isolinux_cfg)
        
    def _copy_nova_files(self, model_path, arch):
        """Copy NOVA kernel and model"""
        print("Copying NOVA files...")
        
        # Copy kernel
        kernel_src = Path(f"build/{arch}/nova-kernel")
        if kernel_src.exists():
            shutil.copy2(kernel_src, self.work_dir / 'nova/kernel')
        else:
            # Create dummy kernel for testing
            (self.work_dir / 'nova/kernel').write_bytes(b'NOVA_KERNEL' * 1000)
            
        # Copy model
        if Path(model_path).exists():
            shutil.copy2(model_path, self.work_dir / 'nova/model.aios')
        else:
            print(f"Warning: Model not found: {model_path}")
            (self.work_dir / 'nova/model.aios').write_bytes(b'MODEL' * 1000)
...
"""
        ...
        # Create cpio archive
        cmd = f"cd {initrd_dir} && find . | cpio -o -H newc | gzip > {self.work_dir}/nova/initrd.img"
    ...
    def _create_boot_config(self, arch, features):
        ...
        import json
        (self.work_dir / 'nova/config.json').write_text(json.dumps(config, indent=2))
...
def main():
    parser = argparse.ArgumentParser(description='Create NOVA bootable ISO')
